...
// Was this plan drawn by hand, generated by the AI, or both? Every
// action the AI applies is flagged aiGenerated:true when it is added
// (PlanWorkspace.js addAiProposalToPlan / addAiResponseProposalToPlan),
// and savePlanIfLoggedIn stores the markers whole -- so the answer is
// already in the saved snapshot and does not need re-deriving. It was
// simply never surfaced, leaving every saved plan looking hand-made.
function planOrigin(plan) {
  const a = plan.actions || {};
  const all = [].concat(a.markers || [], a.closedRoads || [], a.embankments || []);
  if (all.length === 0) return null;
  const ai = all.filter(function (x) { return x && x.aiGenerated; }).length;
  if (ai === 0) return { key: "manual", label: "Drawn by hand", emoji: "✏️", ai: 0, total: all.length };
  if (ai === all.length) return { key: "ai", label: "AI-generated", emoji: "✨", ai: ai, total: all.length };
  return { key: "mixed", label: ai + " of " + all.length + " from AI", emoji: "✨", ai: ai, total: all.length };
}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        {(function () {
                          ...
                          return (
                            <span
                              className={
                                "font-body text-[10px] font-semibold rounded-full px-2 py-0.5 border " +
                                (origin.key === "manual"
                                  ? "border-line text-mint/70"
                                  : "border-flow/40 text-flow bg-flow/10")
                              }
                            >
                      ...
                      <div className="px-6 pb-6 pt-1 border-t border-line">
                        ...
                        {isPreventionSummary(summary) && (
                          <div className="mt-4">
                            ...
                            {/* Protection coverage. The before/after
                                flood-extent figures above are real but
                                barely move for any prevention plan (see
                                Backend/prevention_coverage.py), so a
                                saved plan read as though it achieved
                                nothing. This is the metric that
                                actually changes, and it is already in
                                every save -- it just was not shown. */}
                            {summary.coverage && summary.coverage.at_risk_buildings > 0 && (
                              <div className="mt-4 rounded-lg border border-flow/30 bg-flow/5 px-3 py-3">
                                <div className="font-body text-[11px] uppercase tracking-wide text-flow/80 mb-2">
                                  Protection coverage
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                  <div>
                                    <div className="font-mono text-sm font-bold text-flow">
                                      0% → {summary.coverage.protected_percent_after}%
                                    </div>
                                    <div className="font-body text-[11px] text-mint/70">
                                      of {summary.coverage.at_risk_buildings} flood-exposed buildings protected
                                    </div>
                                  </div>
                                  <div>
                                    <div className="font-mono text-sm font-bold text-flow">
                                      0% → {summary.coverage.cut_roads_percent_after}%
                                    </div>
                                    <div className="font-body text-[11px] text-mint/70">
                                      of {summary.coverage.cut_roads_total} flood-cut road segments covered
                                    </div>
                                  </div>
                                </div>
                                {summary.after && summary.after.local_flood_reduction && (
                                  <div className="mt-3 pt-3 border-t border-flow/20">
                                    <div className="font-mono text-lg font-bold text-flow">
                                      {summary.after.local_flood_reduction.reduction_percent}% of the flooding gone
                                    </div>
                                    <div className="font-body text-[11px] text-mint/70">
                                      on the ground these measures protect &mdash;{" "}
                                      <span className="text-alert">{summary.after.local_flood_reduction.flooded_percent_before}%</span>
                                      {" "}under water before,{" "}
                                      <span className="text-flow">{summary.after.local_flood_reduction.flooded_percent_after}%</span> after
                                    </div>
                                  </div>
                                )}
                                {summary.coverage.treated_channel_m > 0 && (
                                  <div className="font-body text-[11px] text-mint/70 mt-2">
                                    {Math.round(summary.coverage.treated_channel_m).toLocaleString()}m of channel desilted/cleared
                                    ({summary.coverage.treated_channel_percent}% of the network)
                                  </div>
                                )}
                              </div>
                            )}
                              ...
                              {items.map(function (it) {
                                return (
                                  <div key={it.key} className="bg-ink/40 border border-line rounded-lg px-3 py-2">
                                    <div className="font-body text-xs text-paper font-semibold flex items-center gap-2">
                                      <span>{it.emoji} {it.label}</span>
                                      {it.aiGenerated && (
                                        <span className="font-body text-[9px] font-semibold rounded-full px-1.5 py-0.5 border border-flow/40 text-flow bg-flow/10">
                                          ✨ AI
                                        </span>
                                      )}
                                    </div>
